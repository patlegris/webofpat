 ...
 * Version: 1.1
 * Author: Elegant Themes & Mon Site Facile
...
class ET_Automatic_Updates {
	...
	/**
	 * Themes equivalences folderName => theme name on elegantthemes
	 *
	 * @var array
	 */
	var $aThemesMSF = array('basic'         => 'Basic',         'bluesky' => 'BlueSky',             'cion' => 'Cion',
	                        'coldstone'     => 'ColdStone',     'aggregate' => 'Aggregate',         'askit' => 'AskIt',
	                        'bluemist'      => 'BlueMist',      'boutique' => 'Boutique',           'egamer' => 'eGamer',
	                        'envisioned'    => 'Envisioned',    'estore' => 'eStore',               'evid' => 'eVid',
	                        'evolution'     => 'Evolution',     'fable' => 'Fable',                 'flexible' => 'Flexible',
	                        'gleam'         => 'Gleam',         'glider' => 'Glider',               'grungemag' => 'GrungeMag',
	                        'lightsource'   => 'LightSource',   'lucid' => 'Lucid',                 'magnificent' => 'Magnificent',
	                        'memoir'        => 'Memoir',        'nimble' => 'Nimble',               'notebook' => 'Notebook',
	                        'nova'          => 'Nova',          'quadro' => 'Quadro',               'studioblue' => 'StudioBlue',
	                        'thestyle'      => 'TheStyle',      'tidalforce' => 'TidalForce',       'trim' => 'Trim',
	                        'vertex'        => 'Vertex',        'webly' => 'Webly',                 'businesscard' => 'BusinessCard',
	                        'chameleon'     => 'Chameleon',     'convertible' => 'Convertible',
	                        'dailyjournal'  => 'DailyJournal',  'deepfocus' => 'DeepFocus',
	                        'egallery'      => 'eGallery',      'elegantestate' => 'ElegantEstate', 'event' => 'Event',
	                        'explorable'    => 'Explorable',    'feather' => 'Feather',             'foxy' => 'Foxy',
	                        'fusion'        => 'Fusion',        'harmony' => 'Harmony',             'inreview' => 'InReview',
	                        'instyle'       => 'InStyle',       'leanbiz' => 'LeanBiz',             'modest' => 'Modest',
	                        'mycuisine'     => 'MyCuisine',     'myresume' => 'MyResume',           'nexus' => 'Nexus',
	                        'polished'      => 'Polished',      'simplepress' => 'SimplePress',     'simplism' => 'Simplism',
	                        'sky'           => 'Sky',           'styleshop' => 'StyleShop',         'thecorporation' => 'TheCorporation',
	                        '13floor'       => '13Floor',       'artsee' => 'ArtSee',               'bold' => 'Bold',
	                        'cherrytruffle' => 'CherryTruffle', 'dailynotes' => 'DailyNotes',
	                        'delicatenews'  => 'DelicateNews',  'deviant' => 'Deviant',             'earthlytouch' => 'EarthlyTouch',
	                        'ebusiness'     => 'eBusiness',     'elist' => 'eList',                 'enews' => 'eNews',
	                        'ephoto'        => 'ePhoto',        'glow' => 'Glow',                   'influx' => 'Influx',
	                        'interphase'    => 'InterPhase',    'lightbright' => 'LightBright',     'lumin' => 'Lumin',
	                        'minimal'       => 'Minimal',       'myapp' => 'MyApp',                 'myproduct' => 'MyProduct',
	                        'onthego'       => 'OnTheGo',       'origin' => 'Origin',               'personalpress' => 'PersonalPress',
	                        'professional'  => 'Professional',  'puretype' => 'PureType',           'thesource' => 'TheSource',
	                        'whoswho'       => 'WhosWho',       'wooden' => 'Wooden',               'divi' => 'Divi',);


	/**
	 * Plugin defaults credentials
	 *
	 * @var string
	 */
	var $username   = 'infomaniak';
	var $api_key    = '138f1cc01db72d0d493f370fd6cf106b07d0cec3';
	...
	function check_is_active_account() {
		...
		$options = array(
			...
			'headers'    => array( 'Accept-Encoding' => '' ) // MSF attempt to [FIX] gzinflate() error
		);
	...
	function init_options() {
		...
		// if no options, set Mon Site Facile credentials by default
		if(empty($this->options)){
			update_option( 'et_automatic_updates_options', array( 'msf'=> 1, 'username' => $this->username, 'api_key' => $this->api_key ) );
		}
	}
	...
	function add_settings() {
		...
		add_settings_field(
			'et_automatic_updates_msf',
			__( 'Credentials', 'et_automatic_updates' ),
			array( $this, 'show_msf_setting' ),
			'general',
			'et_automatic_updates_section',
			array( 'label_for' => 'et_automatic_updates_options[msf]' )
		);
	...
	/**
	 * Add js custom for plugin
	 */
	public function options_general_et_msf_add_js(){
		?>
		<script>
			//<![CDATA[
			jQuery(document).ready(function($){
				var sRadio      = "input[name='et_automatic_updates_options[msf]']";
				var oUser       = $("input[name='et_automatic_updates_options[username]']");
				var oKey        = $("input[name='et_automatic_updates_options[api_key]']");
				var sUserMsf    = '<?php echo $this->username; ?>';
				var sKeyMsf     = '<?php echo $this->api_key; ?>';

				function showPersonnal(){
					oUser.parent().parent().show();
					oKey.parent().parent().show();
				}

				function hidePersonnal(){
					oUser.parent().parent().hide();
					oKey.parent().parent().hide();
				}

				function isMsf(){
					var iVal = $(sRadio+':checked').val();

					if ( iVal == 1 ){
						hidePersonnal();
						oUser.attr('type', 'password');
						oKey.attr('type', 'password');
						if(sUserMsf == ''){
							sUserMsf    = oUser.val();
						}
						if(oUser.val() != sUserMsf){
							oUser.val(sUserMsf);
						}
						if(sKeyMsf == ''){
							sKeyMsf     = oKey.val();
	 ...
	 * @return void
	 */
	function show_msf_setting() {
		$this->options_general_et_msf_add_js();

		$bMsf = ($this->options['msf']==1 || $this->options['username'] == 'infomaniak')?true:false;

		printf(
			'<p><label><input name="et_automatic_updates_options[msf]" type="radio" value="1" class="tog"%1$s>'.esc_html__( 'Use Mon Site Facile credentials', 'et_automatic_updates' ).'</label>	</p>
				<p><label><input name="et_automatic_updates_options[msf]" type="radio" value="0" class="tog"%2$s>'.esc_html__( 'Use Personal credentials', 'et_automatic_updates' ).'</label></p>',
			( ($bMsf)?' checked="checked"':'' ), ( (!$bMsf)?' checked="checked"':'' )
		);
	}
	...
	function force_update() {
		...
		ET_dd(__METHOD__.':'.__LINE__ );

		$mThemes	= get_site_transient( 'update_themes' );
		$mPlugins	= get_site_transient( 'update_plugins' );


		ET_dd(__METHOD__.': mThemes', $mThemes);
		ET_dd(__METHOD__.': mPlugins',$mPlugins);

		if ( $mThemes )
			$this->et_check_themes_updates( $mThemes );

		if ( $mPlugins )
			$this->check_plugins_updates( $mPlugins );
	}
	...
	function et_check_themes_updates( $update_transient ){
		...
		ET_dd(__METHOD__.': update_transient', $update_transient );

		if ( !isset($update_transient->checked) ) return $update_transient;
		else $themes = $update_transient->checked;


		ET_dd(__METHOD__.': $themes', $themes );

		// MSF: Force la vérification d'update d'un theme Elegant en remettant le nom utilisé par l'api Elegant Themes
		// un nom en minuscule ne fonctionnant pas pour la détection d'update, un slug équivalent est appliqué: divi => Divi
		foreach ($this->aThemesMSF as $key => $value) {
			if( !empty($themes[$key]) && empty($themes[$value]) ){ // fake real theme name
				$themes[$value] = $themes[$key];
			}
		}
		...
		ET_dd(__METHOD__.': send_to_api', $send_to_api );
		...
		if ( ! is_wp_error( $theme_request ) && wp_remote_retrieve_response_code( $theme_request ) == 200 ){
			...
			ET_dd(__METHOD__.': theme_response '.__LINE__, $theme_response );
			...
			if ( ! empty( $theme_response ) ) {
				...
				// change theme key en lowercase
				foreach ( $theme_response as $key => $et_theme ) {
					$theme_response[strtolower($key)]  = $et_theme;
					unset($theme_response[$key]);
				}
...
function ET_dd($sTitle, $mData = null){
	if( isset($_REQUEST['bETDebug']) ){
		echo '<hr/>';
		echo '<h4>'.$sTitle.'</h4>';
		echo '<pre>';
		var_dump($mData);
